...
resource "null_resource" "build_exportS3" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/exportS3/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "exportS3" {
  type        = "zip"
  output_path = "${path.module}/../code/exportS3.zip" # TODO: change here
  source_dir  = "${path.module}/../code/exportS3"     # TODO: change here

  depends_on = [
    null_resource.build_exportS3 # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.exportS3.
...
resource "aws_lambda_function" "exportS3" {
  filename      = data.archive_file.exportS3.output_path
  function_name = "${var.group_name}_${terraform.workspace}_exportS3" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.exportS3.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "exportS3" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.exportS3.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/exportS3" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "exportS3" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.exportS3.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "exportS3" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/exportS3" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.exportS3.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "exportS3_log" {
  name              = "/aws/lambda/${aws_lambda_function.exportS3.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
